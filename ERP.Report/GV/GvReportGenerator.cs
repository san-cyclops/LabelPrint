...
using ERP.Domain;
using ERP.Report.GV.Transactions.Reports;
using ERP.Service;
using ERP.Utility;

namespace ERP.Report.GV
{
    /// <summary>
    /// 
    /// </summary>
    ...
    public class GvReportGenerator
    {
        ...
        public void GenerateTransactionSummaryReport(AutoGenerateInfo autoGenerateInfo, DataTable dtReportData, DataTable dtReportConditions, List<Common.ReportDataStruct> reportDataStructList, List<Common.ReportDataStruct> groupByStructList, List<Common.ReportDataStruct> orderByStructList)
        {
            ...
            switch (autoGenerateInfo.FormName)
            {
                ...
                case "FrmGiftVoucherPurchaseOrder":
                    ...
                    {
                        GvRptGroupedDetailsTemplate gvRptGroupedDetails = new GvRptGroupedDetailsTemplate();
                        reportViewer.crRptViewer.ReportSource = ViewGroupedReport(gvRptGroupedDetails, dtReportData, dtReportConditions, reportDataStructList, autoGenerateInfo);
                    }
                    else
                    {
                        GvRptDetailsTemplate gvRptDetailsTemplate = new GvRptDetailsTemplate();
                        reportViewer.crRptViewer.ReportSource = ViewUnGroupedReport(gvRptDetailsTemplate, dtReportData, dtReportConditions, reportDataStructList, autoGenerateInfo);
                    }
        ...
        public FrmReprotGenerator OrganizeFormFields(AutoGenerateInfo autoGenerateInfo)
        {
            ...
            switch (autoGenerateInfo.FormName)
            {
                ...
                case "FrmGiftVoucherPurchaseOrder":
                    ...
                    reportDatStructList.Add(new Common.ReportDataStruct() { ReportField = "FieldString1", ReportFieldName = "Date", ReportDataType = typeof(string), DbColumnName = "DocumentDate", ValueDataType = typeof(DateTime), IsSelectionField = true, IsConditionField = true, IsGroupBy = true  }); //, IsGroupBy = true
                    ...
                    return frmReprotGenerator = new FrmReprotGenerator(autoGenerateInfo, reportDatStructList, 8, 2, 8);
                #endregion
                #region GiftVoucherGoodsReceivedNote
                case "FrmGiftVoucherGoodsReceivedNote":
                    reportDatStructList = new List<Common.ReportDataStruct>();
                ...
                case "FrmGiftVoucherTransfer":
                    ...
                    reportDatStructList.Add(new Common.ReportDataStruct() { ReportField = "FieldString1", ReportFieldName = "From Location", ReportDataType = typeof(string), DbColumnName = "LocationID", DbJoinColumnName = "LocationName", IsConditionNameJoined = true, ValueDataType = typeof(int), IsJoinField = true, IsSelectionField = true, IsConditionField = true, IsGroupBy = true , IsMandatoryField = true });
                    ...
                    reportDatStructList.Add(new Common.ReportDataStruct() { ReportField = "FieldString3", ReportFieldName = "Date", ReportDataType = typeof(string), DbColumnName = "DocumentDate", ValueDataType = typeof(DateTime), IsSelectionField = true, IsConditionField = true, IsGroupBy = true  }); //, IsGroupBy = true 
                ...
                case "RptGiftVoucherRegister":
                    reportDatStructList = new List<Common.ReportDataStruct>();
                    reportDatStructList.Add(new Common.ReportDataStruct() { ReportField = "FieldString1", ReportFieldName = "Voucher Serial", ReportDataType = typeof(string), DbColumnName = "VoucherSerial", ValueDataType = typeof(string), IsSelectionField = true, IsConditionField = true, IsGroupBy = true, IsRecordFilterByGivenOption = true , IsMandatoryField = true });
                    ...
                    reportDatStructList.Add(new Common.ReportDataStruct() { ReportField = "FieldDecimal2", ReportFieldName = "Value", ReportDataType = typeof(decimal), DbColumnName = "GiftVoucherValue", ValueDataType = typeof(decimal), IsSelectionField = true, IsConditionField = true, IsColumnTotal = true });
        ...
        private GvRptDetailsTemplate ViewUnGroupedReport(GvRptDetailsTemplate gvRptDetailsTemplate, DataTable dtReportData, DataTable dtReportConditions, List<Common.ReportDataStruct> reportDataStructList, AutoGenerateInfo autoGenerateInfo)
        {
            DataTable dtArrangedReportData = new DataTable();
            #region Set Values for report header fields
            strFieldName = string.Empty;
            ...
            foreach (var item in reportDataStructList)
            {
                if (dtReportData.Columns.Contains(item.ReportField.Trim()))
                {
                    ...
                    {
                        strFieldName = "st" + sr;
                        gvRptDetailsTemplate.DataDefinition.FormulaFields[strFieldName].Text = "'" + item.ReportFieldName.Trim() + "'";
                        sr++;
                    }
            ...
            gvRptDetailsTemplate.SetDataSource(dtArrangedReportData);
            gvRptDetailsTemplate.Subreports["RptReportConditions.rpt"].SetDataSource(dtReportConditions);
            gvRptDetailsTemplate.SummaryInfo.ReportTitle = autoGenerateInfo.FormText;
            gvRptDetailsTemplate.SummaryInfo.ReportAuthor = Common.AuthorName + "  " + Common.AuthorAddress;
            gvRptDetailsTemplate.DataDefinition.FormulaFields["LogingUser"].Text = "'" + Common.LoggedUser + "'";
            gvRptDetailsTemplate.DataDefinition.FormulaFields["LogingLocation"].Text = "'" + Common.LoggedLocationName + "'";
            gvRptDetailsTemplate.DataDefinition.FormulaFields["CompanyName"].Text = "'" + Common.LoggedCompanyName + "'";
            gvRptDetailsTemplate.DataDefinition.FormulaFields["Address"].Text = "'" + Common.LoggedCompanyAddress + "'";
            gvRptDetailsTemplate.DataDefinition.FormulaFields["DateFormat"].Text = "'" + Common.DateFormat + "'";

            // Set parameter sum fields
            for (int i = 0; i < gvRptDetailsTemplate.ParameterFields.Count; i++)
            {
                if (gvRptDetailsTemplate.ParameterFields[i].ParameterValueType.Equals(ParameterValueKind.BooleanParameter) && newSumFieldsIndexesList.Contains(i + 1) && gvRptDetailsTemplate.ParameterFields[i].Name.StartsWith("prmSumFieldDecimal"))
                {
                    gvRptDetailsTemplate.SetParameterValue("prmSumFieldDecimal" + (i + 1).ToString(), true);
                }
                else
                {
                    gvRptDetailsTemplate.SetParameterValue("prmSumFieldDecimal" + (i + 1).ToString(), false);
                }
            }

            return gvRptDetailsTemplate;
        ...
        private GvRptGroupedDetailsTemplate ViewGroupedReport(GvRptGroupedDetailsTemplate gvRptGroupedDetails, DataTable dtReportData, DataTable dtReportConditions, List<Common.ReportDataStruct> reportDataStructList, AutoGenerateInfo autoGenerateInfo)
        {
            DataTable dtArrangedReportData = new DataTable();
            ...
            foreach (var col in dtReportData.Columns)
            {
                if (reportDataStructList.Any(c => c.ReportField.Equals(col.ToString())))
                {
                    ...
                    {
                        strFieldName = "st" + sr;
                        gvRptGroupedDetails.DataDefinition.FormulaFields[strFieldName].Text = "'" + reportDataStruct.ReportFieldName.Trim() + "'";
                        sr++;
            ...
            gvRptGroupedDetails.SetDataSource(dtArrangedReportData);
            gvRptGroupedDetails.Subreports["RptReportConditions.rpt"].SetDataSource(dtReportConditions);
            gvRptGroupedDetails.SummaryInfo.ReportTitle = autoGenerateInfo.FormText;
            gvRptGroupedDetails.SummaryInfo.ReportAuthor = Common.AuthorName + "  " + Common.AuthorAddress;
            gvRptGroupedDetails.DataDefinition.FormulaFields["LogingUser"].Text = "'" + Common.LoggedUser + "'";
            gvRptGroupedDetails.DataDefinition.FormulaFields["LogingLocation"].Text = "'" + Common.LoggedLocationName + "'";
            gvRptGroupedDetails.DataDefinition.FormulaFields["CompanyName"].Text = "'" + Common.LoggedCompanyName + "'";
            gvRptGroupedDetails.DataDefinition.FormulaFields["Address"].Text = "'" + Common.LoggedCompanyAddress + "'";
            gvRptGroupedDetails.DataDefinition.FormulaFields["GroupNames"].Text = "'" + (string.IsNullOrEmpty(groupingFields) ? "" : groupingFields) + "'";
            gvRptGroupedDetails.DataDefinition.FormulaFields["DateFormat"].Text = "'" + Common.DateFormat + "'";
           ...
           int ix = 0;
            ...
            for (int i = 0; i < dtArrangedReportData.Columns.Cast<DataColumn>().Where(c => c.ColumnName.StartsWith("FieldString")).Count(); i++)
            {
                if (i < gvRptGroupedDetails.DataDefinition.Groups.Count)
                {
                    gvRptGroupedDetails.DataDefinition.Groups[i].ConditionField = gvRptGroupedDetails.Database.Tables[0].Fields[string.Concat("FieldString", (i + 1).ToString())];
                    ix++;
                }
            }
            ...
            for (int i = 0; i < dtArrangedReportData.Columns.Cast<DataColumn>().Where(c => c.ColumnName.StartsWith("FieldDecimal")).Count(); i++)
            {
                if (i < gvRptGroupedDetails.DataDefinition.Groups.Count)
                {
                    gvRptGroupedDetails.DataDefinition.Groups[ix].ConditionField = gvRptGroupedDetails.Database.Tables[0].Fields[string.Concat("FieldDecimal", (i + 1).ToString())];
                    ix++;
                }
            }

            // Set parameter select field values
            for (int i = 0; i < gvRptGroupedDetails.DataDefinition.Groups.Count; i++)
            {
                if (gvRptGroupedDetails.ParameterFields[i].ParameterValueType.Equals(ParameterValueKind.BooleanParameter))
                {
                    if (i < dtArrangedReportData.Columns.Cast<DataColumn>().Where(c => c.ColumnName.StartsWith("FieldString")).Count())
                    {
                        gvRptGroupedDetails.SetParameterValue("prmSelectGroup" + (i + 1).ToString(), true);
                    }
                    else
                    {
                        gvRptGroupedDetails.SetParameterValue("prmSelectGroup" + (i + 1).ToString(), false);
                    }
                }
            }
