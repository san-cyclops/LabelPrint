...
namespace ERP.UI.Windows
{
    ...
    public partial class FrmReferenceSearch : Form
    {
        ...
        /// <summary>
        /// int to be focussed from search form
        /// </summary>
        public int FocusOrder { get; set; }

        /// <summary>
        /// search index from parent control
        /// </summary>
        [DefaultValue(0)]
        public int DefaultSearchIndex   { get; set ;}

        /// <summary>
        /// oparand index from parent control
        /// </summary>
        [DefaultValue(0)]
        public int DefaultOperandIndex { get; set; } 
        ...
        private void FrmReferenceSearch_Load(object sender, EventArgs e)
        {
            try
            {
                Common.IsSearchFormOpened = true;

                this.Text = FormCaption.Trim();              
                dgvSearch.DataSource = DvResults;
				if (dgvSearch.RowCount > 0)
                {
                	Common.GridAlignment(dgvSearch);
				}
                else
                {
                    Toast.Show("No result found", "", "", Toast.messageType.Information, Toast.messageAction.General).Equals(DialogResult.No);
                }
        ...
        private void dgvSearch_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    FocusControl.Text = dgvSearch.SelectedRows[0].Cells[FocusOrder].Value.ToString().Trim();
                    //FocusControl.KeyDown(sender, e);
                    FocusControl.Focus();
                }
        ...
        private void SetSearchOptions()
        {
            try
            {
                if (DvResults.Table.Rows.Count > 0)
                {
                    ...
                    string a = DvResults.Table.Rows[0].Field<string>(0).ToString();

                   for (int i = 0; i < DvResults.Table.Columns.Count; i++)
                    {
                       if (i==1) 
                       {
                           dgvSearch.Columns[i].Width = (a.Length + 6) * 10;
                       }
                       else
                       {
                           if (a.Length < 10)
                           {
                               dgvSearch.Columns[i].Width = (a.Length + 3) * 10;
                           }
                           else
                           {
                               dgvSearch.Columns[i].Width = (a.Length + 2) * 10;
                           }
                       }
                          
                        dtSearchOption.Rows.Add(i, DvResults.Table.Columns[i].ColumnName.Trim());
                        a = DvResults.Table.Columns[i].ToString();                      
                    }

                    cmbSearchOption.DataSource = dtSearchOption;
                    cmbSearchOption.ValueMember = "GridIndex";
                    cmbSearchOption.DisplayMember = "HeaderColumn";
                    cmbSearchOption.SelectedIndex = DefaultSearchIndex;
        ...
        private void SetOperands()
        {
            try
            {
                ...
                cmbOperation.SelectedIndex = DefaultOperandIndex;
            }
            catch (Exception ex)
            {
                Logger.WriteLog(ex, MethodInfo.GetCurrentMethod().Name.ToString(), this.Name, Logger.logtype.ErrorLog, Common.LoggedLocationID);
        ...
        private void FrmReferenceSearch_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode.Equals(Keys.Escape)) { this.Close(); }
            }
            catch (Exception ex)
            {
                Logger.WriteLog(ex, MethodInfo.GetCurrentMethod().Name.ToString(), this.Name, Logger.logtype.ErrorLog, Common.LoggedLocationID);
        ...
        private void FrmReferenceSearch_FormClosing(object sender, FormClosingEventArgs e)
        {
            Common.IsSearchFormOpened = false;
        }
